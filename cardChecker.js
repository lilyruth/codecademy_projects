...
function validateCred(anArr) {
   let workingArr = anArr.slice();
   let checkDigit = workingArr.pop();
   workingArr.reverse();
   let modulusArr = [];
   
   for (let i = 0; i < workingArr.length; i += 2) {
     let num = workingArr[i] * 2;
     if (num > 9) {
       num -= 9;
     }
     modulusArr.push(num);
   ...
   for (let i = 1; i < workingArr.length; i += 2) {
     modulusArr.push(workingArr[i]);
   }
   let modulusCheck = modulusArr.reduce((acc, red) => acc + red) + checkDigit;
   if (modulusCheck % 10 === 0) return true;
   else return false;
}

function findInvalidCards(nestedArr) {
   invalidBatch = nestedArr.filter( arr => !validateCred(arr));
   return invalidBatch;
}
...
console.log(idInvalidCardCompanies([valid1, valid2]));
